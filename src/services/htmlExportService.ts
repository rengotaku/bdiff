...
<body>
        ...
        <div class="file-comparison">
          <div class="file-info original-file">
            ...
            <div class="file-details">
              <div><strong>名前:</strong> ${this.escapeHtml(originalFile.name)}</div>
              <div><strong>サイズ:</strong> ${originalFile.size.toLocaleString()} bytes</div>
              <div><strong>行数:</strong> ${originalFile.content.split('\\n').length.toLocaleString()}</div>
              ${originalFile.lastModified ? `<div><strong>更新:</strong> ${originalFile.lastModified.toLocaleString('ja-JP')}</div>` : ''}
            </div>
          </div>
          ...
          <div class="file-info modified-file">
            <h3>📄 変更ファイル</h3>
            <div class="file-details">
              <div><strong>名前:</strong> ${this.escapeHtml(modifiedFile.name)}</div>
              <div><strong>サイズ:</strong> ${modifiedFile.size.toLocaleString()} bytes</div>
              <div><strong>行数:</strong> ${modifiedFile.content.split('\\n').length.toLocaleString()}</div>
              ${modifiedFile.lastModified ? `<div><strong>更新:</strong> ${modifiedFile.lastModified.toLocaleString('ja-JP')}</div>` : ''}
            </div>
          </div>
        </div>
      </div>
  ...
  private static generateFooter(): string {
    ...
    <footer class="report-footer">
      ...
      <p class="print-note">💡 このレポートは印刷に最適化されています</p>
    </footer>`;
  }
  ...
  static downloadHtml(htmlContent: string, filename: string): void {
    try {
      ...
      throw new Error('HTMLファイルのダウンロードに失敗しました');
    }
  }
  ...
  static previewHtml(htmlContent: string): void {
    try {
      ...
      if (!previewWindow) {
        throw new Error('ポップアップブロックのためプレビューを開けませんでした');
      }
      
      previewWindow.document.write(htmlContent);
      previewWindow.document.close();
      previewWindow.document.title = 'BDiff エクスポートプレビュー';
    } catch (error) {
      console.error('Error previewing HTML:', error);
      throw new Error('HTMLプレビューの表示に失敗しました');
    }
  }
